package com.kucuk.dw.service.dao;
...
public class MessageDoa {
    ...
    static {
        ...
        CONTENTS = Stream
                .generate(() -> {
                    int begin = random.nextInt(LOREM_IPSUM.length() - 100);
                    return LOREM_IPSUM.substring(begin, begin + 100);
                })
                .limit(1000)
    ...
    public static List<Message> getMessages(int size) {
        return Stream.generate(() -> Message.builder()
                .messageId(Instant.now().toEpochMilli())
